...
if kill -0 "$worker_pid" 2>/dev/null; then
  printf 'worker process exited before cleanup completed\n' >&2
  exit 1
fi
if kill -0 "$agent_pid" 2>/dev/null; then
  printf 'agent process exited before cleanup completed\n' >&2
  exit 1
fi
...
if kill -0 "$worker_pid" 2>/dev/null; then
  printf 'worker process still exists after cleanup\n' >&2
  exit 1
fi
if kill -0 "$agent_pid" 2>/dev/null; then
  printf 'agent process still exists after cleanup\n' >&2
  exit 1
fi
[[ ! -e "$worktree" ]]
