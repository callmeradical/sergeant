...
# First-pass: creates correct v4 cleanup-owner, fails at worktree remove.
run_stable_identity_first_pass head-change "$TEST_ROOT/head-removals"
[[ -f "$head_change_state/cleanup-owner" ]] || {
  printf 'first-pass did not create cleanup-owner for head-change\n' >&2; exit 1
}
printf 'removing\n%s\ngit\n%s\n' \
  "$TEST_ROOT/head-change-sgt-head-change" \
