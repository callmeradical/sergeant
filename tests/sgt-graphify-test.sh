...
grep -Eq 'extract .*/sources/api --out' "$TEST_ROOT/graphify.log"
# After atomic symlink swap the configured symlink points to a new backing dir;
# verify content via the configured path, not the (now-removed) old backing dir.
...
# Use a dedicated subdirectory as the atomic test root so _publish_parent_for_output
# resolves to $at_root (not $TEST_ROOT), keeping leftover-dir checks isolated.
at_root="$TEST_ROOT/atomic-test"
at_bin="$at_root/bin"
at_config="$at_root/config"
at_repo="$at_root/repo"
at_live="$at_root/live"
at_output="$at_root/link"
...
# Also need python3 and yq for the graph-source-prefix and config-parse steps.
ln -s "$(command -v python3)" "$at_bin/python3"
ln -s "$(command -v yq)" "$at_bin/yq"

set +e
SERGEANT_CONFIG="$at_config" PATH="$at_bin:$PATH" \
  "$ROOT_DIR/bin/sgt-graphify" atproject >/dev/null 2>&1 || true
set -e
...
# No leftover temp dirs should remain in the atomic test's output parent (at_root).
leftover=0
for d in "$at_root"/.sgt-graphify-live.* "$at_root"/.sgt-graphify-nextsym.*; do
