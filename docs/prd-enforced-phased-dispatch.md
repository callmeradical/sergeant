...
- **Maintainer or auditor:** diagnoses interrupted work and verifies the relationship among requests, approvals, artifacts, td tasks, fleets, commits, and PRs.
...
6. td and fleet records provide a durable, correlated audit trail without storing artifact bodies, prompts, responses, credentials, or secrets.
...
- Replacing OpenSpec, td, Git, treehouse, worker review, no-mistakes, CI, or repository-native tests.
...
- **Artifact revision:** a manifest of one or more repository paths, their Git commit, and their SHA-256 content digests.
...
- **Owning repository:** the repository where the lifecycle's PRD, canonical td task, and primary audit references live.
...
|---|---|---|
| `request_captured` | Record intent, owning repository, affected repositories, source references, and initial td correlation. | PRD authoring is dispatched, an eligible PRD skip proposal is published directly to `awaiting_prd_approval`, or the lifecycle is cancelled. |
...
| `ready_for_implementation` | Resolve repository decomposition, dependency order, and implementation td tasks from the approved artifacts. After reapproval, reconcile existing workers before creating any replacements. | Implementation workers are successfully dispatched, or matching paused workers are resumed. |
...
- Each phase worker receives a brief limited to its current phase, approved inputs, permitted outputs, repository scope, and td task.
...
- No implementation worktree, worker process, or implementation td start may be created before both gates have durable approval or eligible skip records.
...
2. show the PRD path, Git commit, SHA-256 digest, source request, and downstream effect, or show the proposed skip category and reason;
...
5. append a privacy-safe td decision log referencing the same evidence; and
...
Cancellation requires the same interactive-terminal and local-actor evidence as approval. The command displays the current phase and all nonterminal workers, then requires the human to type `cancel <lifecycle-id>` exactly. It atomically prevents new dispatches, logs a privacy-safe td decision, and enters `cancelling`; it then requests termination through existing supervision and records each worker as stopped or orphaned. It enters `cancelled` only after proving no worker is live. A worker that cannot yet be terminated or classified leaves the lifecycle in `cancelling` with a recoverable blocked condition and exact diagnostics. Repeating the command resumes or reports the same cancellation without creating another transition. Cleanup becomes eligible only after `cancelled` is reached.

## Artifact Immutability

- A phase publishes a canonical artifact manifest before entering its approval gate.
- Each manifest entry contains a repository-relative path, Git commit SHA, and SHA-256 digest. A canonical multi-file OpenSpec manifest also receives one aggregate SHA-256 digest.
- Approval binds to the manifest and artifact bytes, not merely to a branch, path, PR, mutable ref, or latest commit.
- Approved artifact bytes must remain recoverable from the recorded Git commit.
...
## Fleet and td Audit Requirements
...
- project, owning repository, affected repositories, branch, and source td IDs;
...
- The owning td task is the durable human-readable index for the lifecycle.
- The lifecycle has one canonical td task as its durable index. PRD authoring, specification authoring, and each repository's implementation have distinct child td tasks correlated to that canonical task; a source task may become the canonical task but never substitutes for a phase task.
- Significant transitions, approvals, skips, invalidations, recovery decisions, artifact paths, Git commits, and SHA-256 digests are logged to td.
- td handoff is required before a phase worker requests review or becomes terminal.
- PRD and specification child tasks enter td review while awaiting their human gate; they are not closed merely because an artifact was committed. The matching approval command records the human gate evidence and approves/closes that child task before advancing. A rejected or interrupted gate leaves the child task in review, while a superseding revision returns it to active work and requires a new handoff and review.
- Implementation td tasks must not start before `ready_for_implementation`.
- Fleet cleanup preserves the complete privacy-safe lifecycle event ledger, terminal summary, and td references through the existing Sergeant capture mechanism before ephemeral worktrees or live fleet state are removed. Sergeant retains this capture indefinitely and never deletes it automatically; a user may delete it manually under their own retention policy.

Fleet events are the machine-operational record; td is the durable task narrative and index. A mismatch blocks phase advancement and requires recovery rather than silently choosing one record.
...
- On coordinator restart, Sergeant reconstructs current state from durable fleet events, verifies it against td and artifact manifests, and resumes the existing phase. It must not create a replacement lifecycle or duplicate workers automatically.
...
- Fleet lifecycle metadata, td logs, approval events, notifications, and terminal captures must never store PRD or OpenSpec bodies, dispatch brief bodies, prompts, model responses, `.sergeant-response` plaintext, credentials, tokens, environment dumps, or secrets.
...
- New `sgt-dispatch` invocations use the phased lifecycle by default. Existing project, brief, repository, branch, dependency, td, and supported execution-backend inputs remain accepted; the behavioral change is that implementation waits behind gates. Explicit `--read-only` dispatches use enforced read-only mode instead.
...
- Existing td tasks remain valid and can be correlated as source tasks. Sergeant creates or links missing phase tasks without rewriting task history.
...
2. Given an approved PRD revision or skip, no implementation worker, worktree, or implementation td start exists before a successful `sgt-approve-spec` event for the complete OpenSpec revision or eligible specification skip proposal.
...
9. Every phase transition is correlated across fleet state and td by lifecycle ID, phase, generation, artifact digest or skip, and timestamp.
10. Scanning fleet metadata, td logs, notifications, and terminal captures finds no artifact bodies, brief bodies, prompts, responses, credentials, tokens, or secrets from phase fixtures.
...
16. Cleanup rejects every nonterminal lifecycle state and preserves the complete privacy-safe event ledger plus a terminal td-linked summary before deleting ephemeral state.
...
18. Given any nonterminal phase, `sgt-cancel` requires exact interactive confirmation, prevents new dispatch, and records one idempotent td-correlated cancellation event. It remains in nonterminal `cancelling` while any worker is live or unaccounted for and permits cleanup only after reaching `cancelled`.
...
20. PRD and specification child td tasks remain in review at their gates, close through matching approval evidence, and return to active work when a superseding revision invalidates that evidence.
