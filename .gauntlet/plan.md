...
- ShellCheck bootstrap: Phase -2 records existing findings in
  ...
  generated from tracked regular files with shell shebangs/extensions and is
  checked for drift. CI fails on new findings. Phase 6.5 reduces the baseline to
  zero; Phase 8 requires raw ShellCheck green over the same inventory.
- Python bootstrap: `.gauntlet/python-files.txt` is the tracked Python inventory.
  CI runs pinned Ruff and `python3 -m py_compile` over it. Remove the tracked
  `bin/__pycache__/sgt-callbackcpython-312.pyc` and add `__pycache__/`/`*.pyc` to
  `.gitignore`; generated bytecode never enters either inventory.
...
- The same wave keeps `.gauntlet/shell-files.txt` and
  `.gauntlet/python-files.txt` drift-free and clears all Ruff findings. Non-shell
  files are never routed through ShellCheck.
...
python3 scripts/gauntlet/inventory.py --check-control-cards-class \
  --control-file .gauntlet/control-cards.txt
tests/agent-factory-gauntlet.sh --fresh --two-coordinators --all-faults
xargs shellcheck < .gauntlet/shell-files.txt
ruff check $(cat .gauntlet/python-files.txt)
python3 -m py_compile $(cat .gauntlet/python-files.txt)
mise run test:docker:drain
tests/run-gauntlet-tests.sh --docker-bash-3.2
```
...
- 2026-08-05: Critic round 26 found the terminal ShellCheck command included a
...
- Round 26 critic: quality bar won. Phase 8 ran ShellCheck over a Python script
  and a tracked `__pycache__` directory, while Phase -2 explicitly prohibited
