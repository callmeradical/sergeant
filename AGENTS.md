...
- `SERGEANT_AGENT` — override the agent binary used for dispatch. Supported values: `opencode`, `claude`, `goose`. If not set, sergeant auto-detects from the environment (`OPENCODE`/`OPENCODE_PID` → opencode; `CLAUDE_CODE_SESSION_ID` → claude). Goose is selected explicitly, for example `SERGEANT_AGENT=goose goose`. Each agent gets the right non-interactive flags automatically (`opencode run --auto` / `claude --dangerously-skip-permissions` / `goose run --output-format json -t`).
