...
- `sgt-dispatch --model` or `SERGEANT_MODEL` pins the harness model as `provider/model[:variant]`; `--agent`/`SERGEANT_AGENT` select the executable and are orthogonal to it. Precedence is `--model` > `SERGEANT_MODEL` > the harness's ambient default, and an unpinned dispatch is recorded as `unpinned` rather than left blank. A tuple the selected harness cannot honor — a malformed tuple, a variant no harness can pin at launch, or a provider an argv-bare harness cannot address — fails before any intent file, td task, worktree, or fleet state is created, and a worker handed one fails terminally instead of inheriting the ambient default.
