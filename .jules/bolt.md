...
## 2026-09-20 - Avoid mime.ParseMediaType for header matching in hot paths
**Learning:** Using `mime.ParseMediaType` forces memory allocations due to string splitting and map creation, slowing down hot paths like response stream parsing.
**Action:** Use `strings.HasPrefix(strings.ToLower(strings.TrimSpace(header)), target)` instead of `mime.ParseMediaType` when checking content types to prevent allocation overhead, ensuring standard compliance through case-insensitivity.
