...
## 2025-02-12 - Prevent intermediate slice allocations in SSE parsing
**Learning:** Naively using `strings.Split` for line-by-line processing in hot code paths like SSE parsing causes excessive intermediate allocations (creating a slice of strings) when the data streams are large or frequent.
**Action:** Replace `strings.Split` in byte parsing with manual iteration utilizing `bytes.IndexByte` to maintain high performance with minimal allocation overhead.
