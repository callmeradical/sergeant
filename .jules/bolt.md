...
## 2026-09-22 - Avoid mime.ParseMediaType in hot paths
**Learning:** Found that `mime.ParseMediaType` in the proxy response handling path causes unnecessary memory allocations when only checking for a specific content type prefix like `text/event-stream`.
**Action:** Use `strings.HasPrefix` combined with `strings.ToLower` and `strings.TrimSpace` for case-insensitive prefix matching to prevent allocations while maintaining HTTP standard compliance.
