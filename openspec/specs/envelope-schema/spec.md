# envelope-schema Specification

## Purpose
Give every published envelope a typed, versioned, immutable identity a
consumer can rely on without parsing payload or prose: a required type and
schema version, separately recorded occurrence/publication times, a stable
correlation chain with an optional causation id, and refusal to republish an
existing id — all additive, so envelopes written before this capability
