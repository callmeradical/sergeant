# output-redaction Specification

## Purpose
Keep captured agent and gate output safe and bounded to store: strip common
secret shapes before anything is written durably, cap size with visible
truncation rather than silent loss, and always redact before truncating so a
secret can never survive half-cut at the size boundary.
## Requirements
